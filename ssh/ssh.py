...
def ssh(_robot_ip: str, action: str) -> None:
    """Do the work."""
    ...
    with Progress(text_column, bar_column, console=console) as progresso:
        ...
        def progress(filename: str | bytes, size: int, sent: int, address: Tuple[str, int]) -> None:
            """Print progress."""
            ...
            progresso.update(current_task_id, total=size, advance=sent, refresh=True)
            if percent_done >= 100:
        ...
        match action:  # noqa: E999 https://github.com/charliermarsh/ruff/issues/282
            case Action.GET:
                ...
                stdin.close()
                download_task = progresso.add_task("[red]Downloading...")
                current_task_id = download_task
            ...
            case Action.PUT_DB:
                upload_task = progresso.add_task("[red]Uploading...", start=False)
                current_task_id = upload_task
